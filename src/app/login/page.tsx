...
function LoginContent() {
  // 1. Hook para ler parâmetros da URL de forma segura no cliente
  const searchParams = useSearchParams()
  const errorMessage = searchParams.get('error')

  // 2. Controle de transição para o estado de carregamento
  const [isPending, startTransition] = useTransition()

  // 3. Função intermediária que protege a requisição
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center p-4 relative overflow-hidden selection:bg-teal-500 selection:text-white">
      ...
      <div className="w-[90vw] sm:w-full max-w-md p-10 bg-white/75 border border-white rounded-3xl  relative z-10 transform scale-[0.85] sm:scale-100 origin-center">
        ...
        {/* Formulário com a ação interceptada */}
        <form action={handleAction} noValidate className="flex flex-col gap-6">
          ...
          <div className="mt-4">
            <button
              type="submit"
              disabled={isPending}
              className={`w-full py-4 px-4 font-black uppercase tracking-wide rounded-xl transition-all ${
                isPending
                  ? 'bg-emerald-500/50 text-white/60 cursor-not-allowed shadow-none transform-none'
                  : 'bg-gradient-to-r from-teal-500 to-emerald-600 hover:from-teal-400 hover:to-emerald-500 text-white shadow-[0_4px_15px_rgba(16,185,129,0.5)] hover:shadow-[0_6px_20px_rgba(16,185,129,0.7)] transform hover:-translate-y-0.5'
              }`}
            >
              {isPending ? 'Entrando...' : 'Entrar no Jogo'}
            </button>
